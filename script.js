 ...
 */
const NIYYAH_API_URL = (
  typeof window !== 'undefined' && window.location.protocol === 'capacitor:'
) ? 'https://niyyah-api.railway.app'   // ← remplace par ton URL Railway/Render
...
async function scannerCapture() {
  ...
  try {
    ...
    scannerShowResult(result);
  } catch(err) {
    // Fallback offline élégant
    scannerShowResult({ category: 'default', labels: ['objet'] });
  } finally {
...
async function scannerAnalyzeImage(imageData) {
  ...
  try {
    ...
    if (response.ok) {
      const data = await response.json();
      return {
        category: scannerMapLabel(data.labels || [data.object || '']),
        labels: data.labels || [data.object],
        objectName: data.object || 'objet'
      };
    }
  } catch(e) {}
  ...
  try {
      ...
      body: JSON.stringify({
        ...
        messages: [{
          ...
          content: [{
            ...
            type: 'text',
            text: 'Identifie l objet principal dans cette image en 1 à 3 mots en français. Réponds uniquement avec le nom de l objet, rien d autre.'
          }]
        }]
      })
    });
    ...
    if (response.ok) {
      const data = await response.json();
      const objectName = data.content?.[0]?.text?.trim().toLowerCase() || 'objet';
      return {
        category: scannerMapLabel([objectName]),
        labels: [objectName],
...
function scannerConfirmNiyyah() {
  ...
  setTimeout(function() {
    ...
    try { if (typeof applyInfusion === 'function') applyInfusion(nuance.text); } catch(e) {}
...
function scannerRetry() {
  document.getElementById('scanner-result').classList.remove('active');
  document.getElementById('scanner-capture-btn').classList.remove('hidden');
  document.getElementById('scanner-hint').textContent = 'Pointe vers un objet · Capture';
