// ✦ PONT GLOBAL — fonctions appelées depuis HTML
window.scannerShareNiyyah   = function() { if (typeof scannerShareNiyyah === 'function') scannerShareNiyyah(); };
window.scannerConfirmNiyyah = function() { if (typeof scannerConfirmNiyyah === 'function') scannerConfirmNiyyah(); };
window.scannerRetry         = function() { if (typeof scannerRetry === 'function') scannerRetry(); };
window.scannerCapture       = function() { if (typeof scannerCapture === 'function') scannerCapture(); };
window.scannerOpen          = function() { if (typeof scannerOpen === 'function') scannerOpen(); };
window.scannerClose         = function() { if (typeof scannerClose === 'function') scannerClose(); };
...
function renderResume() {
  ...
  el.innerHTML = '<div class="resume-hero">' + ringHtml + '<div class="resume-title">' + msg.title + '</div><div class="resume-sub">' + msg.sub + '</div></div>' +
    ...
    '<div class="resume-actions">' +
      '<button class="btn-new-day" onclick="confirmReset();switchView(\'checklist\')">↺ Nouvelle journée — Bismillah</button>' +
      '<button style="width:100%;padding:14px;border-radius:14px;border:1px solid rgba(255,214,10,0.3);background:rgba(255,214,10,0.06);color:#ffd60a;font-size:15px;font-weight:500;cursor:pointer;font-family:var(--sans);margin-bottom:0;" onclick="openBilanSoir()">🌙 Bilan du soir</button>' +
...
function showWeeklyBilan() {
  ...
  card.innerHTML = `
    ...
    <button class="weekly-btn" onclick="closeWeeklyBilan()" style="width:100%;padding:15px;border-radius:14px;border:none;background:linear-gradient(135deg,var(--gold-v2),#B8940A);color:#1a0f00;font-size:15px;font-weight:700;cursor:pointer;font-family:'Cinzel',serif;letter-spacing:0.05em;">
...
/* ════════════════════════════════════════════════
   SCANNER — GÉNÉRATION CARTE & PARTAGE ✦
   ════════════════════════════════════════════════ */
async function scannerShareNiyyah() {
  // Récupérer l'intention affichée
  const niyyahEl = document.getElementById('scanner-niyyah-main');
  const intention = niyyahEl ? niyyahEl.textContent.replace(/[""]/g, '').trim() : '';
  if (!intention) return;

  const canvas = document.getElementById('scanner-share-canvas');
  const SIZE = 1080;
  canvas.width  = SIZE;
  canvas.height = SIZE;
  const ctx = canvas.getContext('2d');
  ...
  // Halo doré central
  const halo = ctx.createRadialGradient(SIZE/2, SIZE/2, 0, SIZE/2, SIZE/2, SIZE*0.55);
  halo.addColorStop(0,   'rgba(200,168,74,0.10)');
  halo.addColorStop(0.5, 'rgba(200,168,74,0.04)');
  ...
  ctx.lineWidth = 1.5;
  // Coin haut-gauche
  ctx.beginPath(); ctx.moveTo(c, c + cSize); ctx.lineTo(c, c); ctx.lineTo(c + cSize, c); ctx.stroke();
  // Coin haut-droit
  ctx.beginPath(); ctx.moveTo(SIZE-c-cSize, c); ctx.lineTo(SIZE-c, c); ctx.lineTo(SIZE-c, c+cSize); ctx.stroke();
  // Coin bas-gauche
  ctx.beginPath(); ctx.moveTo(c, SIZE-c-cSize); ctx.lineTo(c, SIZE-c); ctx.lineTo(c+cSize, SIZE-c); ctx.stroke();
  // Coin bas-droit
  ctx.beginPath(); ctx.moveTo(SIZE-c-cSize, SIZE-c); ctx.lineTo(SIZE-c, SIZE-c); ctx.lineTo(SIZE-c, SIZE-c-cSize); ctx.stroke();
  ...
  ctx.fillText('بِسْمِ اللهِ الرَّحْمَنِ الرَّحِيمِ', SIZE/2, 158);

  // ── ORNEMENT ────────────────────────────────
  ctx.fillStyle = 'rgba(200,168,74,0.6)';
  ctx.font = '24px serif';
  ctx.fillText('✦  ✦  ✦', SIZE/2, 220);

  // ── INTENTION — texte principal ──────────────
  ctx.fillStyle = '#F0EAD6';
  ctx.textAlign = 'center';
  ctx.textBaseline = 'middle';

  // Guillemets ouvrants
  ctx.font = 'italic 72px "Cormorant Garamond", "Playfair Display", Georgia, serif';
  ctx.fillStyle = 'rgba(200,168,74,0.5)';
  ctx.fillText('“', SIZE/2 - 360, 310);

  // Wrapper le texte
  const maxWidth = SIZE - 180;
  const lineHeight = 88;
  const words = intention.split(' ');
  let lines = [];
  let currentLine = '';

  ctx.font = 'italic 58px "Cormorant Garamond", "Playfair Display", Georgia, serif';
  ctx.fillStyle = '#F0EAD6';
  ...
  const textStartY = SIZE/2 - totalTextHeight/2 + 20;
  ...
  ctx.fillText('”', SIZE/2 + lastLineWidth/2 + 30, lastLineY + 10);

  // ── SÉPARATEUR ───────────────────────────────
  const sepY = SIZE - 220;
  ctx.strokeStyle = 'rgba(200,168,74,0.4)';
  ctx.lineWidth = 1;
  ctx.beginPath(); ctx.moveTo(margin + 80, sepY); ctx.lineTo(SIZE/2 - 30, sepY); ctx.stroke();
  ctx.beginPath(); ctx.moveTo(SIZE/2 + 30, sepY); ctx.lineTo(SIZE - margin - 80, sepY); ctx.stroke();
  ctx.fillStyle = 'rgba(200,168,74,0.8)';
  ctx.font = '18px serif';
  ctx.fillText('✦', SIZE/2, sepY);

  // ── LOGO NIYYAH ──────────────────────────────
  ctx.font = '500 32px "Cinzel", "Cormorant Garamond", serif';
  ctx.fillStyle = 'rgba(200,168,74,0.85)';
  ctx.letterSpacing = '4px';
  ctx.fillText('✦  NIYYAH DAILY', SIZE/2, SIZE - 155);

  ctx.font = '300 22px "Nunito", sans-serif';
  ctx.fillStyle = 'rgba(200,168,74,0.45)';
  ctx.fillText('nabs881-sketch.github.io/niyyah-app', SIZE/2, SIZE - 110);

  // ── PARTAGE ───────────────────────────────────
  canvas.toBlob(async function(blob) {
    const file = new File([blob], 'niyyah.png', { type: 'image/png' });

    // Essayer Web Share API (mobile)
    if (navigator.share && navigator.canShare && navigator.canShare({ files: [file] })) {
      try {
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = 'niyyah.png';
    a.click();
    URL.revokeObjectURL(url);
    v2ShowToast('✦ Carte sauvegardée — partage-la sur WhatsApp');
  }, 'image/png');
}
...
window.scannerShareNiyyah   = scannerShareNiyyah;
