...
## But
Installer un filet de sécurité minimal : un contrôle d'intégrité du contenu qui tourne
au début de chaque build et **bloque le build** si quelque chose casse (JSON cassé,
RDV Sîra manquant, du'a/hadith sans source, etc.). Aucun refactor du code. Déjà testé.

3 actions : créer le fichier, l'ajouter à `npm test`, le brancher dans `build.sh`.

---

## 1. Créer `scripts/validate.mjs`

Créer le dossier `scripts/` s'il n'existe pas, puis le fichier `scripts/validate.mjs`
...
Dans `package.json`, dans la section `"scripts"`, ajouter (ou remplacer un `"test"` placeholder) :
...
Dans `build.sh`, **juste après la ligne `set -e`** (et avant la minification), insérer :
...
Grâce à `set -e`, si la validation échoue (exit 1), le build s'arrête net : un build cassé
ne peut plus être généré ni commit.
...
npm test
```
Doit afficher la liste verte (53 JSON valides, Sîra 365, etc.) et finir par « ✅ Intégrité OK. ».

Test négatif (prouver que ça mord) — ajoute du texte bidon à la fin d'un .json, relance
`npm test` → doit afficher « ❌ BUILD BLOQUÉ » et sortir en erreur. Puis annule la modif.

`npm run build` doit maintenant lancer la validation en tout premier.

## Commit (pas de minif nécessaire — c'est de l'outillage, pas un asset client)
```
git add -A
git commit -m "ci: filet de test integrite contenu (bloque le build si JSON/Sira/source casse)"
git push
```
