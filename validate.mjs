...
function readJSON(p) {
  ...
  return JSON.parse(raw); // jette si JSON cassé
}

// 1) TOUS les JSON parsent (un fichier cassé = écran blanc en prod)
...
// 2) Sîra = 365 RDV, aucun trou, aucun doublon
try {
...
// 3) Du'âs: chaque item a une source non vide + id unique
try {
...
// 4) Hadiths: chaque item a une source non vide
try {
...
// 5) Service worker: ligne VERSION présente et bien formée
try {
...
// 6) Suivi audit (info seulement, ne bloque pas): marqueurs "à confirmer par l'imam"
try {
  let imamCount = 0;
  for (const f of jsonFiles) {
