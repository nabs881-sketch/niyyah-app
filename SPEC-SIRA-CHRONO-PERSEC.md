# SPEC-SIRA-CHRONO-PERSEC

## Objectif
Corriger une incohérence chronologique autour de la persécution des faibles (RDV 22 « Bilâl, Aḥad ») et de l'appel public (RDV 23 « La voix sur Ṣafâ »). Problème : RDV 22 (placé avant Ṣafâ) montre à la fois Bilâl (torturé tôt, son maître l'ayant découvert) **et** Sumayya / les Yâsir (martyre et persécution ORGANISÉE par Abû Jahl) — or cette dernière date de la prédication **publique**, donc d'après Ṣafâ. Et RDV 23 affirmait « tout était secret » juste après.

Vérifié : les esclaves sans tribu (Bilâl, Sumayya…) étaient persécutés par leurs maîtres dès que la conversion se savait dans le foyer ; **mais la persécution sanglante et le martyre de Sumayya datent du début de la prédication publique**.

Deux retouches, **aucune réorganisation des RDV** :
- **RDV 23 [1]** : l'appel n'était pas encore public, mais un esclave ne peut rien cacher à son maître → Bilâl payait déjà.
- **RDV 22 [30]** : signaler que la persécution tourne au sang surtout **quand l'appel devient public et que Quraych se dresse**.

Cible : **data/sira.min.json**, RDV num 23 et 22. `source` inchangé → badges inchangés.

---

## Script (à créer et exécuter)

Crée **`scripts/fix-chrono-persec.js`** puis lance `node scripts/fix-chrono-persec.js` :
...
## Vérif attendue
```
node -e "const d=require('./data/sira.min.json'); const g=n=>JSON.stringify(d.rdv.find(x=>x.num===n)); console.log('RDV23:', /un esclave ne peut rien cacher à son maître/.test(g(23))&&!/l.islam était resté caché/.test(g(23)), '| RDV22:', /tournerait au sang surtout quand l.appel deviendrait public/.test(g(22)), '| total:', d.rdv.length)"
```
...
npm run build
git add -A
git commit -m "sira: chronologie persecution (RDV23 maitre/esclave + RDV22 sang apres appel public)"
```
