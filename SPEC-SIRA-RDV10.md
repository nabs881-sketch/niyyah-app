...
## Objectif
Corriger entièrement le RDV 10 « L'ombre d'Abdul-Muttalib » après vérification (web + sources classiques) :
1. **[0]** : retirer la précision non sourcée « deux mois, dix jours » (l'âge 8 est attesté, le reste non).
2. **[3]-[6]** : l'istisqâ avec l'enfant est du **sîra faible** (Ibn ʿAsâkir) → le donner comme tradition, pas comme fait.
3. **[7]-[8]** : le **poème d'Abû Tâlib est authentique (Bukhârî)** mais décrit le Prophète ﷺ **adulte** → détacher de l'enfance + corriger « un jeune » → *abyaḍ* (« au visage clair »).
4. **[15]** : retirer l'**anachronisme ʿAlî** (pas encore né quand l'enfant de 8 ans entre chez Abû Tâlib).

Cible : **data/sira.min.json**, RDV num 10. Le champ `source` reste inchangé (le badge « Sîra » reste correct : le récit de la remise à Abû Tâlib est du sîra solide, l'istisqâ est flaggé dans le texte).

---

## Script (à créer et exécuter)

Crée **`scripts/fix-rdv10.js`** puis lance `node scripts/fix-rdv10.js` :
...
## Vérif attendue
- `node -e "const d=require('./data/sira.min.json'); const r=d.rdv.find(x=>x.num===10); const t=JSON.stringify(r); console.log('ʿAli retiré:', !/place qu'occupait son fils/.test(t), '| poème corrigé:', /au visage clair/.test(t), '| istisqâ flaggée:', /tradition tardive/.test(t), '| âge:', !/deux mois, dix jours/.test(t))"` → `true | true | true | true`.
...
npm run build
git add -A
git commit -m "sira: RDV10 corrige (istisqa = tradition, poeme recontextualise adulte, anachronisme Ali retire, age)"
```
